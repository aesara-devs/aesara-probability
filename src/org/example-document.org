...
code block to be rendered as raw Markdown, HTML, etc., on export, just set the
...
Alternatively, one can use the customizations in the [[https://github.com/brandonwillard/org-btw][org-btw]] Emacs package.
...
If you're using the default ~org-ref~ package, try the following:
